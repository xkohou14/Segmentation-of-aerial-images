...
\newcommand\vldbavailabilityurl{https://gitlab.com/xkohou15/KNN_segmentation}
% whether page numbers should be shown or not, use 'plain' for review versions, 'empty' for camera ready
\newcommand\vldbpagestyle{plain} 

\begin{document}
\title{Using Lightweight Neural Networks in Segmentation of Aerial Images}
...
\email{xkohou15@stud.fit.vutbr.cz}
...
\email{xkohou14@stud.fit.vutbr.cz}
%%
%% The abstract is a short summary of the work to be presented in the
%% article.
\begin{abstract}
In last decade, the world faces a new challenge -- exponential increasing of data to analyze. With the increasing of the data, requirements for effective data processing increase too because the analysis is not the task for huge data centres anymore. We can analyze data on many devices -- from supercomputers to mobile devices. Huge neural networks are not sufficient for analysis on the second named devices so lightweight neural networks appear on the scene.

In this article, we're going to experiment with lightweight neural network in the analysis of aerial images. For comparison we use UNet network that is classical network used in segmentation tasks. We try to achieve good results in both architectures and we compare these networks in the size and computational efficiency. 
\end{abstract}
...
\begin{figure}
  \centering
  \includegraphics[width=\linewidth]{figures/img_uvodka.png}
  \caption{Example of segmentation tasks.}
  \label{fig:uvodka}
\end{figure}
